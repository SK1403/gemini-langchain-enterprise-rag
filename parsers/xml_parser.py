...
#       into clean semantic text chunks.
#
##
# Development date    Developed by       Comments
# ----------------    ------------       ---------
# 14/11/2024          Saddam Khan        Initial implementation
# 18/12/2024          Saddam Khan        Added XML node traversal and structured attribute extraction
#
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
class XMLParser(BaseParser):
    ...
    def parse_text(self, content: str, filename: str, **kwargs) -> List[DocumentChunk]:
        ...
        try:
            ...
            # Fallback for ill-formed XML or partial fragments
        ...
        # Check if root has repeating child elements (e.g., records, items, transactions)
        children = list(root)
        if len(children) > 1 and len(set(c.tag for c in children)) <= 3:
            # Repeating records schema: chunk per record
            ...
            # Hierarchical document: chunk by major sections
            if len(children) > 0:
                for idx, child in enumerate(children, start=1):
